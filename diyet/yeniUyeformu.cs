...
namespace veritabanifabrika
{
    public partial class yeniUye : Form
    {
        ...
        void uyeEkle()
        {
            ...
            string maas = textBoxMaas.ToString();
            string dogumtarihi = maskedtxtDogumTarihi.ToString();
            string cinsiyet = "";
            ...
            try
            {
                ...
                {
                    // SQL sorgusu gönder
                    cmd.CommandText = "INSERT INTO uyeler WHERE tc = @tc AND sifre = @sifre AND isim = @isim AND soyisim = @soyisim AND maas = @maas AND dogumtarihi = @dogumtarihi AND cinsiyet = @cinsiyet";
                    cmd.Parameters.AddWithValue("@tc", tc);
